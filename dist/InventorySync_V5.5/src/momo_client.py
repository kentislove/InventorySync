        ...
        try:
            ...
            while True:
                ...
                if response.status_code == 200:
                    ...
                    logger.info(f"Fetched page {page}, {len(items)} items. Total: {len(all_inventory)}")
            ...
            for item in all_inventory:
                ...
                if int(qty) > 0:
                    final_inventory.append({
                        "sku": item['part_no'], # Use our SKU as primary identifier
                        "momo_sku": item['sku'],
                        "name": item['name'],
                        "quantity": int(qty),
                        "platform": "MOMO",
                        "goodsdt_info": item['goodsdt_info'],
                        "spec_name": f"/{item['goodsdt_info']}" if item['goodsdt_info'] and not item['goodsdt_info'].startswith('/') else item['goodsdt_info'], # Normalize format
